...
public class XWikiBulletedListInterpreterV09 extends AbstractInterpreter 
{
	private static final String BULLETED_LIST_PATTERN 				= "^(\\*+)([\\p{Graph}\\p{Space}]*?)$";
	private static final String BULLETED_LIST_DECODING_PATTERN		= "<bulletList depth=\\\"([\\d])\\\">([^\\p{Space}](?:[^<\\/]*+)*?)</bulletList>";
	
	private static final String BULLETED_LIST_ENCODING_FORM 		= "<bulletList depth=\"#LEVEL#\">#TEXT#</bulletList>";
	
	public XWikiBulletedListInterpreterV09() 
	{
		super();
	...
	{
		Pattern encodingPattern = Pattern.compile(BULLETED_LIST_PATTERN,Pattern.MULTILINE);
	...
	{
		Pattern decodingPattern = Pattern.compile(BULLETED_LIST_DECODING_PATTERN,Pattern.MULTILINE);
